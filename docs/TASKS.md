...
- **`cohort_access_closed`** — a manual, admin-only toggle checked live ahead of the enrollment chain. Corrects a real bug in the original shipped code, where any refund/cancellation auto-revoked the whole cohort's access regardless of reason; access and billing are now treated as separate questions. **Confirmed fixed (2026-09-18 re-check):** `handle_order_refunded()`/`handle_order_cancelled()` don't exist in the current code at all — `WooCommerce::handle_order_status_changed()` is the only status-transition handler left, and it only ever calls `Enrollment::set_cohort_payment_status()`, never enrollment/access. The auto-unenroll-on-refund bug this section describes was resolved (likely during the `Enrollment`/`WooCommerce` split) without ever being marked closed in this file's own task list — corrected here, not a new fix.
...
| 2026-09-24 | Chatbot widget CSS themeable via `--lp-chatbot-*` custom properties, fallback-only (no `:root` default block in `leaderspath.css`) | Lets a host page (a Bricks global variable landing in `:root`, confirmed working) restyle the widget without editing plugin CSS; a literal `:root` block in this file was tried and reverted — it beat the host's override by source order, defeating the mechanism entirely |
