/**
 * Skills List Module entry point.
 *
 * @package LeadersPath
 * @since 0.1.0
 */

import { type Metadata, type ModuleLibrary } from '@divi/types';

import metadata from './module.json';
import { SkillsListEdit } from './edit';
import { SkillsListAttrs } from './types';
import { placeholderContent } from './placeholder-content';

// Styles.
import './style.scss';

/**
 * Skills List Module registration definition.
 *
 * @since 0.1.0
 */
export const skillsListModule: ModuleLibrary.Module.RegisterDefinition<SkillsListAttrs> = {
  // Imported json has no inferred type hence type-cast is necessary.
  metadata: metadata as Metadata.Values<SkillsListAttrs>,
  placeholderContent,
  renderers: {
    edit: SkillsListEdit,
  },
};
