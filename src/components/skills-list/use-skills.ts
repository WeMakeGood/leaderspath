/**
 * Custom hook to fetch skills for VB preview.
 *
 * Fetches skills via the LeadersPath REST API to display
...
/**
 * Fetches skills from the LeadersPath REST API.
 *
 * This hook fetches skills for displaying in the VB.
...
export function useSkills(): UseSkillsResult {
  ...
  async function fetchSkills(): Promise<void> {
    ...
    try {
      ...
      // Use the fallback endpoint that returns first activity's skills.
      const url = new URL(
        `${wpApiSettings.root}leaderspath/v1/activities/skills`,
