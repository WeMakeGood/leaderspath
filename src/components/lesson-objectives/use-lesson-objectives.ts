/**
 * Custom hook to fetch lesson objectives for VB preview.
 *
 * Fetches lesson objectives via the LeadersPath REST API to display
...
export interface LessonObjectivesData {
  lesson_id: number;
  lesson_title: string;
  objectives: string[];
}

interface UseLessonObjectivesResult {
  /**
   * The lesson objectives data.
   */
  data: LessonObjectivesData | null;
...
/**
 * Fetches lesson objectives from the LeadersPath REST API.
 *
 * This hook fetches lesson objectives for displaying in the VB.
 ...
 */
export function useLessonObjectives(): UseLessonObjectivesResult {
  const [data, setData] = useState<LessonObjectivesData | null>(null);
  ...
  /**
   * Fetch the lesson objectives from REST API.
   */
  async function fetchLessonObjectives(): Promise<void> {
    ...
    try {
      ...
      const url = new URL(
        `${wpApiSettings.root}leaderspath/v1/lessons/objectives`,
